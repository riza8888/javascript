...
function steam(arr) {
    ...
    let output = { games: [], subtotal: 0 };
    //   console.log(output["games"]);
    let total = 0;
    for (let i = 0; i < arr.length; i++) {
      // console.log(arr[i]);
      for (let j = 0; j < listGame.length; j++) {
        let perGame = listGame[j];
        //   console.log(perGame.title);
        let gameTitle = perGame.title;
        let gameGenre = perGame.genre;
        let keywordGame = perGame.key;
        if (arr[i] === keywordGame) {
          output.games.push({ title: gameTitle, genre: gameGenre });
  
          output.subtotal+= perGame.price;
        }
      }
    }
